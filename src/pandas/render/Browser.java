...
public class Browser implements Closeable {
    ...
    public static class Tab implements Closeable {
        ...
        private static final String overrideDateAndRandomJs = "var RealDate = Date;\n" +
                "Date = function () {\n" +
        ...
        Tab(Browser browser, String targetId) {
            ...
            sessionId = browser.call("Target.attachToTarget", Map.of("targetId", this.targetId, "flatten", true)).getString("sessionId");
        ...
        public byte[] screenshot(double x, double y, double width, double height, double scale) {
            var clip = Map.of("x", x, "y", y, "width", width, "height", height, "scale", scale);
            return Base64.getDecoder().decode(call("Page.captureScreenshot", Map.of(
                    "format", "jpeg",
                    "clip", clip)).getString("data"));
        }
