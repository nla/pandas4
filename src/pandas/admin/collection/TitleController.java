...
public class TitleController {
    ...
    public TitleController(TitleRepository titleRepository, SubjectRepository subjectRepository, AgencyRepository agencyRepository, CollectionRepository collectionRepository, FormatRepository formatRepository, StatusRepository statusRepository, GatherMethodRepository gatherMethodRepository, GatherScheduleRepository gatherScheduleRepository, IndividualRepository individualRepository, PublisherRepository publisherRepository, PublisherTypeRepository publisherTypeRepository, Config config, EntityManager entityManager) {
        ...
        facets = new Facet[]{
                ...
                new EntityFacet<>("Collection", "collection", "collections.id", collectionRepository::findAllById, Collection::getId, Collection::getFullName),
                ...
                new EntityFacet<>("Owner", "owner", "owner.id", individualRepository::findAllById, Individual::getId, Individual::getName),
                new EntityFacet<>("Publisher", "publisher", "publisher.id", publisherRepository::findAllById, Publisher::getId, Publisher::getName),
                ...
                new EntityFacet<>("Subject", "subject", "subjects.id", subjectRepository::findAllById, Subject::getId, Subject::getName),
                new EntityFacet<>("Subject 2", "subject2", "subjects.id", subjectRepository::findAllById, Subject::getId, Subject::getName)
        };
    }
    ...
    public String search(@RequestParam(name = "q", required = false) String rawQ,
        ...
        var search = session.search(Title.class)
                .where(f -> f.bool(b -> {
                    ...
                    for (Facet facet : facets) {
                        facet.mustMatch(f, b, queryParams);
                    }
                }))
        ...
        for (Facet facet : facets) {
            ...
            if (queryParams.containsKey(facet.param)) continue;
        ...
        for (Facet facet : facets) {
            if (facet instanceof EntityFacet && queryParams.containsKey(facet.param)) {
